...
- **Task 002: 타입 정의 및 인터페이스 설계** ✅ - 완료
  - ✅ TypeScript 인터페이스 및 타입 정의 파일 생성 (lib/types/profile.ts, event.ts, event-view.ts, admin.ts, api.ts)
  - ✅ 프론트엔드 컴포넌트 Props 타입 정의 (별도 Props 파일 대신 도메인/뷰모델 타입을 Task 003~006 컴포넌트가 직접 재사용하는 방식으로 커버)
  - ✅ API 응답 타입 정의 (lib/types/api.ts의 ApiResult<T> 판별 유니언)
  - ✅ 전역 상태 관리 타입 정의 (현재 상태관리 라이브러리 미도입으로 범위 제외 — 필요 시점(Task 004/008)에 추가)
